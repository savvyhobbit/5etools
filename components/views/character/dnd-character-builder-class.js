...
class DndCharacterBuilderClass extends MutableData(PolymerElement) {
  ...
  _addClassDropdownPlaceholder(classes) {
    console.error(classes);
    return classes && Object.values(classes).length ? "Add a Multiclass" : "Add a Class";
  }

  _hpFieldKeyup(e) {
    console.error("_hpFieldKeyup", e);
    if (e.which === 13) {
      e.target.blur();
    }
    if (e.which === 27) {
      e.target.closest('vaadin-integer-field').value = parseInt(e.target.closest('.btn-field').dataset.original);
      e.target.blur();
    }
  }
